...
)
{
  ...
  exec { "Configure calico ipPool for CIDR $ip_pool":
    path => [ '/bin', '/usr/bin' ],
    command => "set -e; ETCD_ENDPOINTS=`/usr/bin/grep ETCD_ENDPOINTS /etc/calico/calico.env` /opt/cni/bin/calicoctl apply -f /etc/calico/ipPool-${ip_pool}.yaml",
    unless => "set -e; ETCD_ENDPOINTS=`/usr/bin/grep ETCD_ENDPOINTS /etc/calico/calico.env` /opt/cni/bin/calicoctl get -f /etc/calico/ipPool-${ip_pool}.yaml | /usr/bin/grep ${ip_pool}/${ip_mask}",
